...
## 7. Yellow Cell Policy

These fields are ALWAYS yellow (no formal source exists for them):

- **Promomats ID (EN/FR)** -- provided ad hoc by client or creative agency, no lookup table
- **Creative Name** -- invented per campaign, no standardized list
- **Custom Tag 3** -- campaign-specific
- **Target URL / Landing Page** -- planner inputs once, drags down in Excel if same across ads
- **Age Demo** -- if not clearly stated in BC targeting text, left blank (not defaulted)
- **Match Type (Search)** -- never in BC

These fields are yellow only when not determinable from the BC:

- Persona, Campaign Type, Objective, Placement, Tactic Type, Buy Type
