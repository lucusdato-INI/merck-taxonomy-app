...
const SOCIAL_COLUMNS = [
  ...
  'Tactic Type',
  'Geo',
  '(Custom Tag 2) Language',
  '(Custom Tag 2) Province',
  'Ad Set',
  'Promomats ID',
  'Content Purposes',
  'Ad Format',
...
export async function generateTrafficSheet(meta: CampaignMeta, rows: TaxonomyRow[]): Promise<void> {
...
function writeSocialTab(wb: ExcelJS.Workbook, meta: CampaignMeta, rows: TaxonomyRow[]): void {
...
function writeDigitalTab(wb: ExcelJS.Workbook, meta: CampaignMeta, rows: TaxonomyRow[]): void {
...
function writeSearchTab(wb: ExcelJS.Workbook, meta: CampaignMeta, rows: TaxonomyRow[]): void {
