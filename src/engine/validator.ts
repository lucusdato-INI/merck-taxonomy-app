...
const v04: Rule = (row) => {
  ...
  if (
    parts.length >= 2 &&
    ['AB', 'BC', 'ON', 'QC', 'SK', 'MB', 'NB', 'NL', 'NS', 'PE', 'NA'].includes(parts[0])
  ) {
    return {
      ruleId: 'V04',
      severity: 'error',
  ...
  if (cf3Parts.length >= 7) {
    ...
    if (!lastPart || lastPart === cf3Parts[0]) {
      return {
        ...
        severity: 'error',
        message:
          'Digital Creative CF3 order must be: CreativeName+Persona+Age+Gender+Province+Language+Platform',
        field: 'adString',
      }
    }
  }
...
const v14: Rule = (row) => {
  ...
  if (site) {
    ...
    if (
      row.adSetString.includes(site) &&
      site !== siteLower &&
      site !== capitalizeFirst(siteLower)
    ) {
      return {
        ruleId: 'V14',
        severity: 'error',
...
export const VALIDATION_RULE_DESCRIPTIONS: { id: string; description: string }[] = [
  {
    id: 'V01',
    description: 'No spaces allowed in Custom Tag 3 values. Auto-sanitize by removing spaces.',
  },
  {
    id: 'V02',
    description: 'Gender at Ad/Creative level must be the full word: "All", "Male", or "Female".',
  },
  {
    id: 'V03',
    description:
      'Gender at Ad Set/Placement level must be the single-letter acronym: "A", "M", or "F".',
  },
  { id: 'V04', description: 'Custom Tag 2 must follow Language+Province order (e.g., "EN+ON").' },
  {
    id: 'V05',
    description:
      'Digital Creative CF3 field order must be: CreativeName+Persona+Age+Gender+Province+Language+Platform.',
  },
  {
    id: 'V06',
    description:
      'Social Ad CF3 field order must be: Persona+Age+Gender+Language+Province+CustomTag3.',
  },
  { id: 'V07', description: 'Influencer campaigns must have Custom Tag 1 = "Social+Influencer".' },
  {
    id: 'V08',
    description: 'Search Campaign Custom Tag 1 must include language suffix (e.g., "Search+EN").',
  },
  { id: 'V09', description: 'Search/Pmax Ad Set source field must always be "google".' },
  {
    id: 'V10',
    description:
      'Taxonomy strings may only contain: letters, numbers, dashes, underscores, and plus signs.',
  },
  {
    id: 'V11',
    description: 'UTM utm_adset must be aligned with Ad Set (without channel/source prefix).',
  },
  { id: 'V12', description: 'UTM utm_content must be aligned with Ad/Creative string.' },
  {
    id: 'V13',
    description: 'Digital Creative must append the platform name as the last CF3 element.',
  },
